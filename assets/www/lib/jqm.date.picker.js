 ...
 * Date: Mon Jan 5 2015 22:38:22 UTC
	...
	$
			.widget(
					"mobile.datebox",
					{
						...
						options : {
							// All widget options, including some internal
							// runtime details
						...
						_enhanceDate : function() {
							$
									.extend(
											...
											{
												copy : function(adjust,
														override) {
													// Get a modified copy of
													// the date.
													// First array - Offset the
													// new date by #
													// (position determines date
													// part)
													// Second array - If
													// non-zero, force the new
													// date by #
													// (position determines date
													// part)
													...
													override = $.extend([ 0, 0,
															...
															override);
													return new Date(
															((override[0] > 0) ? override[0]
																	: this
																			.get(0)
																			+ adjust[0]),
															((override[1] > 0) ? override[1]
																	: this
																			.get(1)
																			+ adjust[1]),
															((override[2] > 0) ? override[2]
																	: this
																			.get(2)
																			+ adjust[2]),
															((override[3] > 0) ? override[3]
																	: this
																			.get(3)
																			+ adjust[3]),
															((override[4] > 0) ? override[4]
																	: this
																			.get(4)
																			+ adjust[4]),
															((override[5] > 0) ? override[5]
																	: this
																			.get(5)
																			+ adjust[5]),
															((override[6] > 0) ? override[5]
																	: this
																			.get(6)
																			+ adjust[6]));
												},
												...
												getDWeek : function(type) {
													...
													case 0:
														...
														return Math
																.floor((this
																		.getTime() - (t1
																		.getTime() + ((this
																		.getTimezoneOffset() - t1
																		.getTimezoneOffset()) * 60000))) / 6048e5) + 1;
													case 1:
														t1 = this
																.copy(
																		[
													...
													case 4:
														// this line is some
														// bullshit. but it does
														// work.
														// (trap for dec 29, 30,
														// or 31st being in the
														// new year's week -
														// these
														// are the only 3 that
														// can possibly fall
						...
						_event : function(e, p) {
							var tmp, w = $(this).data("mobile-datebox");
						...
						_dur : function(ms) {
							var dur = [ ms / (60 * 60 * 1000 * 24),
									ms / (60 * 60 * 1000) % 24,
									ms / (60 * 1000) % 60, ms / (1000) % 60, ];
						...
						__ : function(val) {
							var o = this.options, lang = o.lang[o.useLang], mode = o[o.mode
									+ "lang"], oride = "override"
						...
						_dRep : function(oper, direction) {
							...
							var ch, i, start = 48, end = 57, adder = 1584, newd = "";
						...
						_n : function(val, def) {
							// Don't allow negative values, use a default
							// instead
						...
						_pa : function(arr, date) {
							...
							if (typeof date === "boolean") {
								return new this._date(arr[0], arr[1], arr[2],
										0, 0, 0, 0);
							}
							return new this._date(date.get(0), date.get(1),
									date.get(2), arr[0], arr[1], arr[2], 0);
						},
						_makeDate : function(str) {
							// Date Parser
							var i, exp_temp, exp_format, grbg, w = this, o = this.options, defVal = this.options.defaultValue, adv = w
									.__fmt(), exp_input = null, exp_names = [], date = new w._date(), d = {
								year : -1,
								mont : -1,
								date : -1,
								hour : -1,
							...
							str = $.trim(((w.__("useArabicIndic") === true) ? w
									._dRep(str, -1) : str));
							...
							if (o.mode === "durationbox"
									|| o.mode === "durationflipbox") {
								adv = adv.replace(/%D([a-z])/gi, function(
										match, oper) {
									switch (oper) {
									case "d":
									case "l":
								...
								if (exp_input === null
										|| exp_input.length !== exp_format.length) {
									if (typeof defVal === "number"
											&& defVal > 0) {
										// defaultValue is an integer
										return new w._date((w.initDate
												.getEpoch() + parseInt(defVal,
												10)) * 1000);
									}
									// No default, use ZERO.
							...
							adv = adv.replace(/%(0|-)*([a-z])/gi, function(
									match, pad, oper) {
								exp_names.push(oper);
								switch (oper) {
								case "p":
							...
							if (exp_input === null
								...
								if (defVal !== false) {
									...
									case "object":
										...
										break;
									case "number":
										date = new w._date(defVal * 1000);
										break;
									case "string":
										if (o.mode.substr(0, 4) === "time") {
											exp_temp = $.extend([ 0, 0, 0 ],
													defVal.split(":")).slice(0,
													3);
								...
								for (i = 1; i < exp_input.length; i++) {
									...
									case "p":
									case "P":
										grbg = new RegExp("^" + exp_input[i]
												+ "$", "i");
										d.meri = (grbg
												.test(w.__("meridiem")[0]) ? -1
												: 1);
										break;
									case "b":
										exp_temp = $.inArray(exp_input[i], w
								...
								date = new w._date(w._n(d.year, 0), w._n(
										d.mont, 0), w._n(d.date, 1), w._n(
										d.hour, 0), w._n(d.mins, 0), w._n(
										d.secs, 0), 0);
						...
						_formatter : function(format, date) {
							...
							format = format
									.replace(
											/%(D|X|0|-)*([1-9a-zA-Z])/g,
											function(match, pad, oper) {
												if (pad === "X") {
													if (typeof w._customformat[o.mode] !== "undefined") {
														return w._customformat[o.mode]
																(oper, date, o);
													}
													return match;
												}
												...
												case "a": // Short Day
													return w
															.__("daysOfWeekShort")[date
															.getDay()];
												case "A": // Full Day of week
													return w.__("daysOfWeek")[date
															.getDay()];
												case "b": // Short month
													return w
															.__("monthsOfYearShort")[date
															.getMonth()];
												case "B": // Full month
													return w.__("monthsOfYear")[date
															.getMonth()];
												case "C": // Century
												...
												case "o": // Ordinals
													if (typeof w._ord[o.useLang] !== "undefined") {
														return w._ord[o.useLang]
																(date.getDate());
													}
													return w._ord["default"]
															(date.getDate());
												case "j":
													tmp = new Date(date
															.getFullYear(), 0,
													...
													tmp = "000"
															+ String(Math
																	.ceil((date - tmp) / 86400000) + 1);
													return tmp.slice(-3);
												case "G":
													tmp = date.getFullYear();
													if (date.getDWeek(4) === 1
															&& date.getMonth() > 0) {
														return tmp + 1;
													}
													if (date.getDWeek(4) > 51
															&& date.getMonth() < 11) {
														return tmp - 1;
													}
													return tmp;
						...
						_minStepFix : function() {
							...
							var newMinute = this.theDate.get(4), mstep = this.options.minuteStep, roundDirection = this.options.minStepRound, remainder = newMinute
									% mstep;
						...
						_offset : function(mode, amount, update) {
							...
							var w = this, o = this.options, now = this.theDate, ok = false;
						...
						_destroy : function() {
							var w = this, o = this.options, button = this.d.wrap
									.find("a");
							...
							w.d.input.off("datebox").off("focus.datebox").off(
									"blur.datebox").off("change.datebox");

							$(document).off(w.drag.eMove).off(w.drag.eEnd).off(
									w.drag.eEndA);
						},
						_create : function() {
							// Create the widget, called automatically by widget
							// system
							$(document).trigger("dateboxcreate");

							var w = this, o = $.extend(this.options, this
									._getLongOptions(this.element),
									this.element.data("options")), thisTheme = ((o.theme === false) ? $.mobile
									.getInheritedTheme(this.element)
									: o.theme), trans = o.useAnimation ? o.transition
									: "none", d = {
								input : this.element,
								...
								mainWrap : $(
										"<div>",
										{
											"class" : "ui-datebox-container ui-overlay-shadow "
													+ "ui-corner-all ui-datebox-hidden "
													+ trans
													+ " ui-body-"
													+ thisTheme
										}).css("zIndex", o.zindex),
								...
								eEndA : (touch ? ([ "mouseup", "touchend",
										"touchcanel", "touchmove" ].join(evtid
										+ " ") + evtid) : "mouseup" + evtid),
								move : false,
								start : false,
								end : false,
							...
							if (o.usePlaceholder !== false) {
								if (o.usePlaceholder === true
										&& w._grabLabel() !== "") {
									w.d.input.attr("placeholder", w
											._grabLabel());
								}
								if (typeof o.usePlaceholder === "string") {
									w.d.input.attr("placeholder",
											o.usePlaceholder);
								}
							}
							...
							w.theDate = (o.defaultValue) ? w._makeDate()
									: ((w.d.input.val() !== "") ? w
											._makeDate(w.d.input.val())
											: new w._date());
							...
							// Disable when done if element attribute disabled
							// is true.
						...
						applyMinMax : function(refresh, override) {
							...
							var todayClean, fromEl, fromElDate, daysRaw, w = this, o = this.options, today = new this._date(), lod = 24 * 60 * 60 * 1000;
							...
							if ((override === true || o.minDays === false)
								...
								fromElDate = new w._date(fromEl[0],
										fromEl[1] - 1, fromEl[2], 0, 0, 0, 0);
								daysRaw = (fromElDate.getTime() - todayClean
										.getTime())
										/ lod;
						...
						open : function() {
							...
							var w = this, o = this.options, popopts = {
								transition : (o.useAnimation ? o.transition
										: "none")
							}, basepop = {
								history : false,
								transition : (o.useAnimation ? o.transition
							...
							if ((o.useInline || o.useInlineBlind)
								...
								if (o.hideContainer) {
									if (o.useHeader) {
										w.d.mainWrap
												.prepend($("<div class='ui-header ui-bar-"
														+ o.themeHeader
														+ "'>"
														+ "<h1 class='ui-title'>"
														+ w.d.headerText
								...
								w.d.mainWrap
										.removeClass("ui-datebox-hidden ui-overlay-shadow");
								if (o.useInline) {
									...
									if (!o.hideInput && !o.hideContainer) {
										w.d.mainWrap
												.addClass("ui-datebox-inline-has-input");
									}
									// This is really hacky. I hate it, but I
									// don't have a
									// better idea right now. Cleans up position
									// on flip variants.
									...
									w.d.mainWrap
											.addClass(
													"ui-datebox-inline ui-datebox-inline-has-input")
											.css("zIndex", "auto");
									w.d.mainWrap.hide();
								}
								w.initDone = false;
							...
							if (o.useHeader) {
								w.d.mainWrap
										.append($("<a href='#'>Close</a>")
												.addClass(
														"ui-btn-left ui-link ui-btn ui-btn-"
																+ ((o.themeCloseButton === false) ? o.themeHeader
																		: o.themeCloseButton)
																+ " ui-icon-delete "
								...
								w.d.mainWrap
										.append($("<div class='ui-header ui-bar-"
												+ o.themeHeader
												+ "'>"
												+ "<h1 class='ui-title'>"
												+ w.d.headerText
							...
							w.d.mainWrap.append(w.d.intHTML).css("zIndex",
									o.zindex);
							w._t({
								method : "postrefresh"
							});
							...
							// Perpare open callback, if provided. Additionally,
							// if this
							...
							// Perpare BEFORE open callback, if provided.
							...
							w.d.mainWrap.removeClass("ui-datebox-hidden")
									.popup(basepop).popup("open", popopts);
						},
						close : function() {
						...
						_check : function() {
							...
							var td, year, month, date, i, w = this, o = this.options, now = this.theDate;
							...
							if (o.mode.substr(0, 4) === "time"
								...
								if (o.blackDatesRec !== false) {
									...
									for (i = 0; i < o.blackDatesRec.length; i++) {
										if ((o.blackDatesRec[i][0] === -1 || o.blackDatesRec[i][0] === year)
						...
						_getLongOptions : function(element) {
							...
							for (key in element.data()) {
								if (key.substr(0, prefixLength) === prefix
									...
									temp = temp.charAt(0).toLowerCase()
											+ temp.slice(1);
									if (temp !== "options") {
						...
						getTheDate : function() {
							// Provide a PUBLIC function to get the current
							// date.
						...
						getLastDur : function() {
							// Provide a PUBLIC function to get the last entered
							// duration
							return this.lastDuration;
						},
						dateVisible : function() {
							// Provide a PUBLIC function to see if selected
							// calendar date is visible
							return this.calDateVisible;
						},
						setTheDate : function(newDate) {
							// Provide a PUBLIC function to set the date
							// ACCEPTS: Date Object or String in proper output
							// format
						...
						getOption : function(opt) {
							// Provide a PUBLIC function to get a defined option
							// or i18n member
...
/*
 * jQuery-Mobile-DateBox Date: Mon Jan 5 2015 22:38:22 UTC
 * http://dev.jtsage.com/jQM-DateBox/
 * https://github.com/jtsage/jquery-mobile-datebox
 * 
 * Copyright 2010, 2015 JTSage. and other contributors Released under the MIT
 * license.
 * https://github.com/jtsage/jquery-mobile-datebox/blob/master/LICENSE.txt
 * 
 */
...
(function($) {
	...
	$
			.extend(
					$.mobile.datebox.prototype,
					{
						...
						_cal_check : function(checkDates, year, month, date,
								done) {
							var w = this, i, o = this.options, maxDate = done.x, minDate = done.i, thisDate = done.t, presetDay = done.p, day = new this._date(
									year, month, date, 0, 0, 0, 0).getDay(), bdRec = o.blackDatesRec, hdRec = o.highDatesRec, ret = {
								ok : true,
								iso : year + "-" + w._zPad(month + 1) + "-"
										+ w._zPad(date),
								theme : o.themeDate,
								force : false,
								recok : true,
							...
							if ($.isArray(o.enableDates)
									&& $.inArray(ret.iso, o.enableDates) < 0) {
								ret.ok = false;
							} else if (checkDates) {
								if ((ret.recok !== true)
										...
										|| (o.maxDays !== false && maxDate
												.comp() < ret.comp)
										|| (o.minDays !== false && minDate
												.comp() > ret.comp)
										|| ($.isArray(o.blackDays) && $
												.inArray(day, o.blackDays) > -1)
										|| ($.isArray(o.blackDates) && $
												.inArray(ret.iso, o.blackDates) > -1)) {
									ret.ok = false;
								}
							}

							if ($.isArray(o.whiteDates)
									&& $.inArray(ret.iso, o.whiteDates) > -1) {
								ret.ok = true;
							}
							...
							if (ret.ok) {
								...
								} else if (o.calHighToday
										&& ret.comp === thisDate.comp()) {
									ret.theme = o.themeDateToday;
								} else if (o.calHighPick && w.calDateVisible
										&& w.calBackDate !== false
								...
								} else if ($.isArray(o.highDates)
										&& ($.inArray(ret.iso, o.highDates) > -1)) {
									ret.theme = o.themeDateHigh;
								} else if ($.isArray(o.highDays)
										&& ($.inArray(day, o.highDays) > -1)) {
									ret.theme = o.themeDayHigh;
								} else if ($.isArray(o.highDatesRec)
										&& ret.rectheme === true) {
	...
	$
			.extend(
					$.mobile.datebox.prototype._build,
					{
						"calbox" : function() {
							var tempVal, pickerControl, calContent, genny, weekdayControl, listControl, row, col, rows, cols, htmlRow, i, prangeS, prangeL, fmtRet, fmtObj, absStartDO, absEndDO, w = this, o = this.options, dList = o.calDateList, uid = "ui-datebox-", curDate = ((w.calBackDate !== false
									&& w.theDate.get(0) === w.calBackDate
											.get(0) && w.theDate.get(1) === w.calBackDate
									.get(1)) ? new w._date(w.calBackDate
									.getTime()) : w.theDate), checked = false, checkDatesObj = {}, minDate = w.initDate
									.copy(), maxDate = w.initDate.copy(), cStartDay = (curDate
									.copy([ 0 ], [ 0, 0, 1 ]).getDay()
									- w.__("calStartDay") + 7) % 7, curMonth = curDate
									.get(1), curYear = curDate.get(0), curDateArr = curDate
									.getArray(), presetDate = (w.d.input.val() === "") ? w
									._startOffset(w._makeDate(w.d.input.val()))
									: w._makeDate(w.d.input.val()), presetDay = -1, cTodayDate = new w._date(), cTodayDateArr = cTodayDate
									.getArray(), weekNum = curDate.copy([ 0 ],
									[ 0, 0, 1 ]).adj(
									2,
									(-1 * cStartDay)
											+ (w.__("calStartDay") === 0 ? 1
													: 0)).getDWeek(4), weekModeSel = 0, isTrueMonth = false, isTrueYear = false, cMonthEnd = 32 - w.theDate
									.copy([ 0 ], [ 0, 0, 32, 13 ]).getDate(), cPrevMonthEnd = 32 - w.theDate
									.copy([ 0, -1 ], [ 0, 0, 32, 13 ])
									.getDate(), checkDates = (o.afterToday
									|| o.beforeToday || o.notToday || o.maxDays
									|| o.minDays || o.blackDays || o.blackDates) ? true
									: false;

							if (w.calBackDate !== false) {
								if (w.theDate.get(0) === w.calBackDate.get(0)
										&& w.theDate.get(1) === w.calBackDate
												.get(1)) {
									w.theDate = new w._date(w.calBackDate
											.getTime());
									w.calBackDate = false;
								}
							}
							...
							w.d.headerText = ((w._grabLabel() !== false) ? w
									._grabLabel() : w
							...
							// Previous and next month buttons, define booleans
							// to decide if they should do anything
							$(
									"<div class='" + uid + "gridplus"
											+ (w.__("isRTL") ? "-rtl" : "")
											+ "'><a href='#'>"
											+ w.__("nextMonth") + "</a></div>")
									.prependTo(
											w.d.intHTML.find("." + uid
													+ "gridheader"))
									.find("a")
									.addClass(
											"ui-btn-inline ui-link ui-btn ui-btn-"
													+ o.themeDate
													+ " ui-icon-"
													+ o.calNextMonthIcon
													+ " ui-btn-icon-notext ui-shadow ui-corner-all")
							...
							if (o.calNoHeader) {
								if (o.calUsePickersIcons) {
									w.d.intHTML.find("." + uid + "gridlabel")
											.hide();
								} else {
							...
							if (Math.floor(cTodayDate.comp() / 100) === Math
									.floor(curDate.comp() / 100)) {
								isTrueMonth = true;
							}
							if (Math.floor(cTodayDate.comp() / 10e3) === Math
									.floor(curDate.comp() / 10e3)) {
								isTrueYear = true;
							}
							...
							if (o.minDays !== false) {
								...
								if (curDateArr[0] === tempVal[0]
										&& curDateArr[1] <= tempVal[1]) {
									w.calPrev = false;
								}
							}
							if (o.maxDays !== false) {
							...
							if (o.calUsePickers) {
								...
								if (o.calNoHeader && o.calUsePickersIcons) {
									pickerControl
											.addClass("ui-datebox-pickicon");
								}

								pickerControl.i = $("<fieldset>").appendTo(
										pickerControl);

								pickerControl.a = $("<select>").appendTo(
										pickerControl.i);
								pickerControl.b = $("<select>").appendTo(
										pickerControl.i);
								...
								pickerControl.a.on("change", function() {
									if (w.calBackDate === false) {
										w.calBackDate = new Date(w.theDate
												.getTime());
									}
									w.theDate.setD(1, $(this).val());
									if (w.theDate.get(1) !== parseInt($(this)
											.val(), 10)) {
										w.theDate.setD(2, 0);
									}
									if (w.calBackDate !== false) {
								...
								pickerControl.b.on("change", function() {
									...
									if (w.theDate.get(1) !== parseInt(
											pickerControl.a.val(), 10)) {
										w.theDate.setD(2, 0);
									}
									if (w.calBackDate !== false) {
										w._t({
								...
								pickerControl.i.find(
										".ui-controlgroup-controls").css({
									marginRight : "auto",
									marginLeft : "auto",
									width : "100%",
									display : "table",
							...
							calContent = $("<div class='" + uid + "grid'>")
									.appendTo(w.d.intHTML);

							if (o.calShowDays) {
								w._cal_days = w.__("daysOfWeekShort").concat(
										w.__("daysOfWeekShort"));
								weekdayControl = $("<div>", {
									"class" : uid + "gridrow"
								}).appendTo(calContent);

								if (o.calControlGroup) {
									weekdayControl.addClass(uid
											+ "gridrow-last");
								}
								if (w.__("isRTL") === true) {
							...
							genny = w._cal_gen(cStartDay, cPrevMonthEnd,
									cMonthEnd, !o.calOnlyMonth, curDate.get(1));
							...
							absStartDO = new Date(w.theDate.get(0),
									genny[0][0][1], genny[0][0][0], 0, 0, 0, 0);
							absEndDO = new Date(w.theDate.get(0),
									genny[genny.length - 1][6][1],
									genny[genny.length - 1][6][0], 0, 0, 0, 0);
							...
							if (w.calBackDate === false) {
								...
								if (o.calOnlyMonth) {
									...
									if (w.calBackDate.comp() < absStartDO
											.comp()
											|| w.calBackDate.comp() > absEndDO
													.comp()) {
										w.calDateVisible = false;
									} else {
										w.calDateVisible = true;
							...
							for (row = 0, rows = genny.length; row < rows; row++) {
								...
								if (o.calShowWeek) {
									...
									weekNum++;
									if (weekNum > 52
											&& typeof (genny[row + 1]) !== "undefined") {
										weekNum = new w._date(
												curDateArr[0],
												curDateArr[1],
												(w.__("calStartDay") === 0) ? genny[row + 1][1][0]
														: genny[row + 1][0][0],
												0, 0, 0, 0).getDWeek(4);
									}
								}
								for (col = 0, cols = genny[row].length; col < cols; col++) {
									...
									if (typeof genny[row][col] === "boolean") {
										...
										checked = w._cal_check(checkDates,
												curDateArr[0],
												genny[row][col][1],
												genny[row][col][0],
												checkDatesObj);
										if (genny[row][col][0]) {
											if (!$.isFunction(o.calFormatter)) {
												...
												fmtObj = {
													"Year" : ((genny[row][col][1] > 11) ? curYear + 1
															: (genny[row][col][1] < 0) ? curYear - 1
																	: curYear),
													"Month" : ((genny[row][col][1] === 12) ? 0
															: (genny[row][col][1] === -1) ? 11
																	: genny[row][col][1]),
												...
												fmtObj.ISO = fmtObj.Year
														+ "-"
														+ w
																._zPad(fmtObj.Month + 1)
														+ "-"
														+ w._zPad(fmtObj.Date);
												fmtObj.Comp = parseInt(
														fmtObj.ISO.replace(
																/-/g, ""), 10);
											...
											$("<div>")
													...
													.addClass(
															uid
																	+ "griddate ui-corner-all ui-btn")
													.addClass(
															(curMonth === genny[row][col][1] || checked.force) ? ("ui-btn-"
																	+ checked.theme + (checked.ok ? ""
																	: " ui-state-disabled"))
																	: (uid + "griddate-empty"))
													.addClass(fmtRet["class"])
													.css(
															(curMonth !== genny[row][col][1] && !o.calOnlyMonth) ? {
																cursor : "pointer"
															}
																	: {})
													.data(
															"date",
															((o.calWeekMode) ? weekModeSel
																	: genny[row][col][0]))
													.data("enabled", checked.ok)
													.data(
															"month",
															genny[row][((o.calWeekMode) ? o.calWeekModeDay
																	: col)][1])
													.appendTo(htmlRow);
										}
									}
							...
							if (o.calShowDateList && dList !== false) {
								...
								for (i = 0; i < dList.length; i++) {
									listControl.a.append($("<option value='"
											+ dList[i][0] + "'>" + dList[i][1]
											+ "</option>"));
								}
								...
								listControl.a.on("change", function() {
									...
									w.theDate = new w._date(tempVal[0],
											tempVal[1] - 1, tempVal[2], 0, 0,
											0, 0);
									w._t({
										method : "doset"
									});
							...
							if (o.useTodayButton || o.useTomorrowButton
									|| o.useClearButton) {
								htmlRow = $("<div>", {
									"class" : uid + "controls"
								});
							...
							w.d.intHTML.on(o.clickEventAlt, "div." + uid
									+ "griddate", function(e) {
								e.preventDefault();
								if ($(this).data("enabled")) {
									w.calBackDate = false;
									w.theDate.setD(2, 1).setD(1,
											$(this).jqmData("month")).setD(2,
											$(this).data("date"));
									w._t({
										method : "set",
										value : w._formatter(w.__fmt(),
							...
							if (w.wheelExists) { // Mousewheel operations, if
													// plugin is loaded
...
(function($) {
	...
	$
			.extend(
					...
					{
						_dbox_run : function() {
							var w = this, g = this.drag, timer = 150;
						...
						_dbox_fixstep : function(order) {
							// Turn back off steppers when displaying a less
							// precise
						...
						_dbox_run_update : function(shortRun) {
							var w = this, o = this.options, i = w.theDate
									.getTime()
									- w.initDate.getTime(), dur = (o.mode === "durationbox" ? true
									: false), cDur = w._dur(i < 0 ? 0 : i);
							...
							if (dur) {
								if (o.minDur !== false
										&& (w.theDate.getEpoch() - w.initDate
												.getEpoch()) < o.minDur) {
									w.theDate = new Date(w.initDate.getTime()
											+ (o.minDur * 1000));
											...
											function() {
												...
												case "M":
													$(this)
															.val(
																	w
																			.__("monthsOfYearShort")[w.theDate
																			.get(1)]);
													break;
												case "a":
													$(this)
						...
						_dbox_vhour : function(delta) {
							var w = this, o = this.options, tmp, closeya = [
									25, 0 ], closenay = [ 25, 0 ];
						...
						_dbox_enter : function(item) {
							...
							if (item.val() !== ""
									&& item.val().toString().search(/^[0-9]+$/) === 0) {
								...
								case "m":
									w.theDate.setD(1,
											parseInt(item.val(), 10) - 1);
									break;
								case "d":
									w.theDate.setD(2, parseInt(item.val(), 10));
									t += (60 * 60 * 24)
											* parseInt(item.val(), 10);
									break;
								case "h":
	...
	$
			.extend(
					$.mobile.datebox.prototype._build,
					{
						...
						"datebox" : function() {
							var offAmount, i, y, tmp, w = this, g = this.drag, o = this.options, dur = (o.mode === "durationbox" ? true
									: false), cnt = -2, defDurOrder = [ "d",
									"h", "i", "s" ], uid = "ui-datebox-", divBase = $("<div>"), divPlus = $("<fieldset>"), divIn = divBase
									.clone(), divMinus = divPlus.clone(), divLab = divBase
									.clone(), inBase = $(
									"<div><input type='text'></div>")
									.addClass(
											"ui-input-text ui-body-"
													+ o.themeInput
													+ " ui-corner-all ui-mini ui-shadow-inset"), butBase = $("<div></div>"), butClass = "ui-btn-inline ui-link ui-btn ui-btn-"
									+ o.themeButton
									+ " ui-btn-icon-notext ui-shadow ui-corner-all";
							...
							w.d.headerText = ((w._grabLabel() !== false) ? w
									._grabLabel() : ((o.mode === "datebox") ? w
									.__("titleDateDialogLabel") : w
									.__("titleTimeDialogLabel")));
							w.d.intHTML = $("<span>");

							w.fldOrder = ((o.mode === "datebox") ? w
									.__("dateFieldOrder") : ((dur) ? w
									.__("durationOrder") : w
									.__("timeFieldOrder")));
							...
							if (!dur) {
								...
								w._minStepFix();
								w
										._dbox_vhour(typeof w._dbox_delta !== "undefined" ? w._dbox_delta
												: 1);
							} else {
								w.dateOK = true;
								...
								if (w.fldOrder[i] !== "a"
									...
									$("<div>")
											.append(
													...
													"ui-block-" + tmp)
											.appendTo(divLab);
									$("<div>").append(w._makeEl(inBase, {
										"attr" : {
											"field" : w.fldOrder[i],
											"amount" : offAmount
										}
									})).addClass("ui-block-" + tmp).appendTo(
											divIn).find("input").data({
										"field" : w.fldOrder[i],
										"amount" : offAmount
									});
									w._makeEl(butBase, {
										"attr" : {
											...
											uid + "cbut ui-block-" + tmp
													+ " ui-icon-plus "
													+ butClass).appendTo(
											divPlus).prepend(
											(dur) ? "<label>"
													+ w.__("durationLabel")[i]
													+ "</label>" : "");
									w._makeEl(butBase, {
										"attr" : {
											"field" : w.fldOrder[i],
							...
							if (dur) {
								divLab.addClass(
										"ui-datebox-dboxlab ui-grid-"
												+ w._gridblk.b[cnt]).appendTo(
										w.d.intHTML);
							}

							divPlus.addClass("ui-grid-" + w._gridblk.b[cnt])
									.appendTo(w.d.intHTML);

							divIn.addClass(
									"ui-datebox-dboxin ui-grid-"
											+ w._gridblk.b[cnt]).appendTo(
									w.d.intHTML);

							divMinus.addClass("ui-grid-" + w._gridblk.b[cnt])
									.appendTo(w.d.intHTML);
							...
							if (w.dateOK !== true) {
								divIn.find("input")
										.addClass("ui-state-disable");
							} else {
								divIn.find(".ui-state-disable").removeClass(
										"ui-state-disable");
							}
							...
							if (!o.repButton) {
								w.d.intHTML.on(o.clickEvent,
										"." + uid + "cbut", function(e) {
											divIn.find(":focus").blur();
											e.preventDefault();
											w._dbox_delta = ($(this).data(
													"amount") > 1) ? 1 : -1;
											w._offset($(this).data("field"), $(
													this).data("amount"));
										});
							}
							...
							if (w.wheelExists) { // Mousewheel operation, if
													// plugin is loaded
							...
							if (o.repButton) {
								w.d.intHTML.on(g.eStart, "." + uid + "cbut",
										function() {
											divIn.find(":focus").blur();
											tmp = [ $(this).data("field"),
													$(this).data("amount") ];
											...
											if (!w.runButton) {
												g.target = tmp;
												w.runButton = setTimeout(
														function() {
															w._dbox_run();
														}, 500);
											}
										});
								w.d.intHTML.on(g.eEndA, "." + uid + "cbut",
										function(e) {
											if (g.move) {
												e.preventDefault();
...
(function($) {
	...
	$
			.extend(
					...
					{
						_fbox_pos : function() {
							var pos1, ech, top, fixer, w = this, par = this.d.intHTML
									.find(".ui-datebox-flipcontent")
									.innerHeight();

							w.d.intHTML.find(".ui-datebox-flipcenter").each(
									function() {
										ech = $(this);
										top = ech.innerHeight();
										ech.css("top",
							...
							w.d.intHTML
									...
									.each(
											function() {
												...
												fixer = ech.find("li").last()
														.offset().top
														- ech.find("li")
																.first()
																.offset().top;
												pos1 = (((fixer - par) / 2) + top
														.outerHeight())
														* -1;
						...
						_fbox_series : function(middle, side, type, neg) {
							...
							var nxt, prv, o = this.options, maxval = (type === "h") ? 24
									: 60, ret = [ [ middle.toString(), middle ] ];
						...
						_fbox_mktxt : {
							...
							m : function(i) {
								var testDate = (this.theDate.copy([ 0 ], [ 0,
										0, 1 ])).adj(1, i);
								return this.__("monthsOfYearShort")[testDate
										.get(1)];
							},
							d : function(i) {
							...
							h : function(i) {
								var testDate = this.theDate
										.copy([ 0, 0, 0, i ]);
								return ((this.__("timeFormat") === 12) ? testDate
										.get12hr()
										: testDate.get(3));
							},
							i : function(i) {
								return this._zPad((this.theDate.copy([ 0, 0, 0,
										0, i ])).get(4));
							}
						}
					});
	$
			.extend(
					$.mobile.datebox.prototype._build,
					{
						...
						"flipbox" : function() {
							var i, y, hRow, tmp, hRowIn, stdPos, w = this, o = this.options, g = this.drag, cDurS = {}, normDurPositions = [
									"d", "h", "i", "s" ], dur = (o.mode === "durationflipbox" ? true
									: false), uid = "ui-datebox-", flipBase = $("<div class='ui-overlay-shadow'><ul></ul></div>"), ctrl = $(
									"<div>", {
										"class" : uid + "flipcontent"
									}), ti = w.theDate.getTime()
									- w.initDate.getTime(), cDur = w
									._dur(ti < 0 ? 0 : ti), currentTerm, currentText;
							...
							w.d.headerText = ((w._grabLabel() !== false) ? w
									._grabLabel() : ((o.mode === "flipbox") ? w
									.__("titleDateDialogLabel") : w
									.__("titleTimeDialogLabel")));
							w.d.intHTML = $("<span>");

							$(document).one("popupafteropen", function() {
								// This fixes bad positioning on initial open -
								// not found a way around this yet.
								w._fbox_pos();
							});

							w.fldOrder = (o.mode === "flipbox") ? w
									.__("dateFieldOrder") : (dur) ? w
									.__("durationOrder") : w
									.__("timeFieldOrder");
							...
							if (dur) {
								...
								tmp = $("<div class='" + uid
										+ "header ui-grid-"
										+ w._gridblk.g[w.fldOrder.length]
										+ "'></div>");
								...
								w.dateOK = true;
								cDurS.d = w._fbox_series(cDur[0], 16, "d",
										false);
								cDurS.h = w._fbox_series(cDur[1], 16, "h",
										(cDur[0] > 0));
								cDurS.i = w._fbox_series(cDur[2], 20, "i",
										(cDur[0] > 0 || cDur[1] > 0));
								cDurS.s = w
										._fbox_series(
												cDur[3],
												20,
												"s",
												(cDur[0] > 0 || cDur[1] > 0 || cDur[2] > 0));

								ctrl.addClass(uid + "flipcontentd");

								for (y = 0; y < w.fldOrder.length; y++) {
									stdPos = w.fldOrder[y];
									currentTerm = cDur[$.inArray(stdPos,
											normDurPositions)];
									...
									for (i in cDurS[stdPos]) {
										$(
												...
												.addClass(
														"ui-body-"
																+ ((cDurS[stdPos][i][1] !== currentTerm) ? o.themeDate
																		: o.themeDatePick))
												.appendTo(hRowIn);
									}
									hRow.appendTo(ctrl);
								...
								hRow = w
										._makeEl(
												flipBase,
												{
													"attr" : {
														"field" : currentTerm,
														"amount" : (currentTerm === "i") ? o.minuteStep
																: 1
													}
												});
								hRowIn = hRow.find("ul");
							...
							if (w.wheelExists) { // Mousewheel operation, if
													// plugin is loaded
								w.d.intHTML.on("mousewheel",
										".ui-overlay-shadow", function(e, d) {
											e.preventDefault();
											w._offset($(this).data("field"),
													((d < 0) ? -1 : 1)
															* $(this).data(
													...
													g.pos = parseInt(
															g.target
																	...
																	.replace(
																			...
																			""),
															10);
													g.start = w.touch ? e.originalEvent.changedTouches[0].pageY
															: e.pageY;
													g.end = false;
													g.direc = (dur) ? -1 : 1;
													e.stopPropagation();
	...
	$
			.extend(
					$.mobile.datebox.prototype._drag,
					{
						"timeflipbox" : function() {
						...
						"flipbox" : function() {
							...
							$(document)
									.on(
											...
											function(e) {
												if (g.move
														&& o.mode.slice(-7) === "flipbox") {
													g.end = w.touch ? e.originalEvent.changedTouches[0].pageY
															: e.pageY;
													g.target
															.css(
																	"marginTop",
...
(function($) {
			...
			.extend(
					$.mobile.datebox.prototype,
					{
						_sbox_pos : function() {
							...
							w.d.intHTML
									.find("div.ui-datebox-sliderow-int")
									.each(
											function() {
												ech = $(this);
						...
						_sbox_mktxt : {
							...
							m : function(i) {
								...
								return [
										"slidemonth",
										this.__("monthsOfYearShort")[testDate
												.get(1)] ];
							},
							d : function(i) {
								...
								return [
										"slideday",
										testDate.get(2)
												+ "<br /><span class='ui-datebox-slidewday'>"
												+ this.__("daysOfWeekShort")[testDate
														.getDay()] + "</span>" ];
							},
							h : function(i) {
								var testDate = this.theDate
							...
							i : function(i) {
								return [
										"slidemins",
										this._zPad((this.theDate.copy([ 0, 0,
												0, 0, i ])).get(4)) ];
							}
						}
					});
	$
			.extend(
					...
					{
						"slidebox" : function() {
							var i, y, hRow, phRow, currentTerm, currentText, w = this, o = this.options, g = this.drag, uid = "ui-datebox-", slideBase = $("<div class='"
									+ uid + "sliderow-int'></div>"), phBase = $("<div>"), ctrl = $(
									"<div>", {
										"class" : uid + "slide"
									});
							...
							for (y = 0; y < w.fldOrder.length; y++) {
								...
								phRow = phBase.clone().addClass(
										uid + "sliderow").data("rowtype",
										currentTerm);

								hRow = slideBase.clone().data("rowtype",
										currentTerm).appendTo(phRow);
								...
								if (typeof w._sbox_mktxt[currentTerm] === "function") {
									for (i = o.slen[currentTerm] * -1; i < (o.slen[currentTerm] + 1); i++) {
										currentText = w._sbox_mktxt[currentTerm]
												.apply(w, [ i ]);
										...
										$(
												"<div>",
												{
													"class" : uid
															...
															+ ((i === 0) ? o.themeDatePick
																	: o.themeDate)
												}).html(currentText[1]).data(
												"offset", i).appendTo(hRow);
									}
									phRow.appendTo(ctrl);
								}
							}
							...
							if (o.useSetButton || o.useClearButton) {
								...
								if (o.useSetButton) {
									y.append(w._stdBtn.close.apply(w, [ w
											.__("setDateButtonLabel") ]));
								}
								if (o.useClearButton) {
									y.append(w._stdBtn.clear.apply(w));
															...
															._offset(
																	...
																	((d < 0) ? -1
																			...
																			* ($(
																					this)
																					.data(
																							"rowtype") === "i" ? o.minuteStep
																					: 1));
												});
							}

							w.d.intHTML.on(o.clickEvent,
									".ui-datebox-sliderow-int>div",
									function(e) {
										e.preventDefault();
										w._offset($(this).parent().data(
												"rowtype"), parseInt($(this)
												.data("offset"), 10));
									});
													...
													g.pos = parseInt(
															g.target
																	...
																	.replace(
																			...
																			""),
															10);
													g.start = w.touch ? e.originalEvent.changedTouches[0].pageX
															: e.pageX;
													g.end = false;
													e.stopPropagation();
													e.preventDefault();
							...
							$(document)
									.on(
											...
											function(e) {
												if (g.move
														&& o.mode === "slidebox") {
													g.end = w.touch ? e.originalEvent.changedTouches[0].pageX
															: e.pageX;
													g.target
															.css(
																	"marginLeft",
...
(function($) {
	...
	$.extend($.mobile.datebox.prototype, {
		"_customflipDoSet" : function() {
			// If this function exists, it overrides the "doset" method of the
			// "datebox" event.
		...
		"_cubox_offset" : function(fld, amount) {
			...
			var w = this, tmp, o = this.options;

			tmp = (w.customCurrent[fld] + amount)
					% o.customData[fld].data.length;
			if (tmp < 0) {
		...
		"_cubox_arr" : function(data, choice) {
			var base = data, x, len = this.options.customDataLen, before = data
					.slice(0, choice), after = data.slice(choice + 1);
		...
		"_cubox_pos" : function() {
			var pos1, ech, top, fixer, w = this, par = this.d.intHTML.find(
					".ui-datebox-flipcontent").innerHeight();
			...
			w.d.intHTML.find("ul").each(
					function() {
						ech = $(this);
						par = ech.parent().innerHeight();
						top = ech.find("li").first();
						fixer = ech.find("li").last().offset().top
								- ech.find("li").first().offset().top;
	...
	$
			.extend(
					$.mobile.datebox.prototype._parser,
					{
						// If this stucture exists, it is called instead of the
						// usual date input parser.
						// The name of the structure is the same as the mode
						// name - it recieves a string
						// as the input, which is the current value of the input
						// element, pre-sanitized
						"customflip" : function(str) {
							var o = this.options, adv = o.customFormat, exp_input, exp_format, tmp, tmp2, retty_val = [
									0, 0, 0, 0, 0, 0 ];
							...
							adv = adv.replace(/%X([0-5a-f])/gi, function(match,
									oper) {
								switch (oper) {
								case "a":
								case "b":
							...
							if (exp_input !== null) {
								for ( var x = 1; x < exp_input.length; x++) {
									...
									if (isNaN(parseInt(tmp))) {
										tmp2 = $.inArray(tmp, [ "a", "b", "c",
												"d", "e", "f" ]);
										retty_val[tmp2] = $.inArray(
												exp_input[x],
												o.customData[tmp2].data);
									} else {
							...
							return (str.length < 1 || retty_val.length < 1) ? o.customDefault
									: retty_val;

						}
					});
	$.extend($.mobile.datebox.prototype._customformat, {
		// If this stucture exists, the formatter will call it when it
		// encounters a special string
		// %X<whatever> - it recieves the single letter operater, and the
		// current "date" value
	...
	$
			.extend(
					$.mobile.datebox.prototype._build,
					{
						// This builds the actual interface, and is called on
						// *every* refresh. (flip triggers)
						"customflip" : function() {
							var i, y, hRow, hRowIn, tmp, lineArr, w = this, o = this.options, uid = "ui-datebox-", customCurrent = this
									._makeDate(this.d.input.val()), flipBase = $("<div class='ui-overlay-shadow'><ul></ul></div>"), ctrl = $(
									"<div>", {
										"class" : uid + "flipcontent"
									});
							...
							if (typeof o.customData === "string"
									&& typeof window[o.customData] !== "undefined") {
								// Allow the passing of a global variable name
								// from data-options or
								// data-datebox-custom-data. The other option
								// was to eval() the data, but that
							...
							w.d.headerText = ((o.customHead !== false) ? o.customHead
									: ((w._grabLabel() !== false) ? w
											._grabLabel() : ""));
							w.d.intHTML = $("<span>");
							...
							if (o.customData.length > 1) {
								tmp
										.addClass("ui-grid-"
												+ [ 0, 0, "a", "b", "c" ][o.customData.length]);
							}
							for (y = 0; y < o.customData.length; y++) {
								$("<div>").addClass(
										(o.customData.length > 1) ? "ui-block-"
												+ [ "a", "b", "c", "d" ][y]
												: "")
										.text(o.customData[y].name).css(
												"textAlign", "center")
										.appendTo(tmp);
							}
							tmp.appendTo(w.d.intHTML);
							...
							for (y = 0; y < o.customData.length; y++) {
								lineArr = w._cubox_arr(o.customData[y].data,
										w.customCurrent[y]);
								...
								for (i = 0; i < lineArr.length; i++) {
									tmp = (i !== o.customDataLen) ? o.themeOpt
											: o.themeOptPick;
									console.log(i + " " + o.customDataLen);
									console.log(typeof i + " "
											+ typeof o.customDataLen);
									$("<li>", {
										"class" : "ui-body-" + tmp
													...
													w.drag.pos = parseInt(
															w.drag.target
																	...
																	.replace(
																			...
																			""),
															10);
													w.drag.start = w.touch ? e.originalEvent.changedTouches[0].pageY
															: e.pageY;
													w.drag.end = false;
													e.stopPropagation();
													e.preventDefault();
	...
	$
			.extend(
					$.mobile.datebox.prototype._drag,
					{
						// This contains the code that the drag and drop (or
						// touch move) code uses
						"customflip" : function() {
							var w = this, o = this.options, g = this.drag;
...
/*
 * jQuery-Mobile-DateBox Date: Wed Nov 19 2014 21:05:33 UTC
 * http://dev.jtsage.com/jQM-DateBox/
 * https://github.com/jtsage/jquery-mobile-datebox
 * 
 * Copyright 2010, 2014 JTSage. and other contributors Released under the MIT
 * license.
 * https://github.com/jtsage/jquery-mobile-datebox/blob/master/LICENSE.txt
 * 
 */
